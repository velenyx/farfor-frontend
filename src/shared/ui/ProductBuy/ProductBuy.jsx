...
export const ProductBuy = ({ product, checkedFirst, isTooltip }) => {
  const twoPrice = checkedFirst
    ? product.sizes[0].price
    : product.sizes[1].price
  const twoPriceDiscount = checkedFirst
    ? product.sizes[0].discount_price
    : product.sizes[1].discount_price

  const twoWeight = checkedFirst
    ? product.sizes[0].weight
    : product.sizes[1].weight
  return (
    <>
      ...
      <div className={styles.weight}>
        {product.sizes.length === 1 && (
          <div className={styles.weight__count}>
            {product.sizes[0].weight + ' г'}
          </div>
        )}
        {product.sizes.length > 1 && (
          <div className={styles.weight__count}>{twoWeight + ' г'}</div>
        )}
        {isTooltip && (
